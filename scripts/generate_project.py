...
for folder, target, product_type in [('Samgloyim', 'samgloyim', 'application'), ('SamgloyimTests', 'SamgloyimTests', 'bundle.unit-test'), ('SamgloyimUITests', 'SamgloyimUITests', 'bundle.ui-testing')]:
    ...
    phases = [add('sources:' + target, f'isa = PBXSourcesBuildPhase; buildActionMask = 2147483647; files = {sequence(builds)}; runOnlyForDeploymentPostprocessing = 0;'),
              add('frameworks:' + target, 'isa = PBXFrameworksBuildPhase; buildActionMask = 2147483647; files = (); runOnlyForDeploymentPostprocessing = 0;'),
    ...
    dependencies = []
    if target == 'samgloyim':
        settings.update({'ASSETCATALOG_COMPILER_APPICON_NAME': 'AppIcon', 'ASSETCATALOG_COMPILER_GLOBAL_ACCENT_COLOR_NAME': 'AccentColor', 'INFOPLIST_KEY_CFBundleDisplayName': 'samgloyim', 'INFOPLIST_KEY_LSApplicationCategoryType': 'public.app-category.finance', 'INFOPLIST_KEY_UILaunchScreen_Generation': 'YES', 'INFOPLIST_KEY_UIApplicationSceneManifest_Generation': 'YES', 'INFOPLIST_KEY_UISupportedInterfaceOrientations_iPhone': 'UIInterfaceOrientationPortrait', 'INFOPLIST_KEY_UISupportedInterfaceOrientations_iPad': 'UIInterfaceOrientationPortrait UIInterfaceOrientationLandscapeLeft UIInterfaceOrientationLandscapeRight', 'CURRENT_PROJECT_VERSION': '1', 'MARKETING_VERSION': '1.0', 'ENABLE_PREVIEWS': 'YES'})
    else:
    ...
    targets.append(add('target:' + target, f'isa = PBXNativeTarget; buildConfigurationList = {configs}; buildPhases = {sequence(phases)}; buildRules = (); dependencies = {sequence(dependencies)}; name = {q(target)}; productName = {q(target)}; productReference = {product}; productType = {q("com.apple.product-type." + product_type)};'))
